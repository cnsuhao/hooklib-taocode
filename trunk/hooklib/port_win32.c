...
int port_make_writeable( const void* buffer,int size )
{
	DWORD dwOldProtect;
	int ret;
	if( 0 == VirtualProtect( (LPVOID)buffer,size,PAGE_EXECUTE_READWRITE, &dwOldProtect) )
...
int port_is_bad_read_ptr( const void* ptr,int size )
{
	return IsBadReadPtr( (void*)ptr,(UINT_PTR)size );	
}

int port_is_bad_write_ptr( void* ptr,int size )
{
	return IsBadWritePtr( (void*)ptr,(UINT_PTR)size );
}
